...
export default function Respond() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (Object.keys(answers).length !== session.questions.length) return;
  ...
  if (isLoading) return <div className="min-h-screen bg-background flex items-center justify-center text-textMuted">Loading connection...</div>;
      ...
      <div className="w-full max-w-3xl mt-8 mb-24 space-y-10">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          {session.questions.length === 0 && (
             <div className="text-center py-10 text-textMuted">This session has no open questions.</div>
          )}

          <div className="pt-8">
            <button 
              type="submit"
              disabled={isSubmitting || Object.keys(answers).length !== session.questions.length || session.questions.length === 0}
